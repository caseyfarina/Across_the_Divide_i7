...
public class mixamoBodySpawn : MonoBehaviour
{
    [Range(0f, 100f)]
    ...
    */
    ...
    void spawnOnLimb(List<GameObject> Limb)
    {
        ...
        //make the spawn a child of the base component of the chain selection
        thisTarget = Instantiate(targetPrefab, lerpPosition, transform.rotation);
        thisTarget.transform.SetParent(Limb[chainsOffset + 1].transform);
        //thisOne.transform.SetParent(Limb[chainsOffset+1].transform);
        thisOne.GetComponent<part_parent_target>().MagnetObjects = thisTarget;
